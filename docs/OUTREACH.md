...
| 3 | **Kijiji alerts (free)** | On kijiji.ca use **Services → Skilled Trades / Services Wanted** with demand phrases (`"looking for"`, `"need a quote"`) — **not** bare `deck`/`windows` in Buy & Sell (that pulls house listings). Full recipes: `docs/LEAD_INTAKE.md`. Also Craigslist / Facebook / HomeStars email alerts. | Free | — |
