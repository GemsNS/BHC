...
const getParameterDefinitions = () => [
  ...
  { name: 'showAccent', type: 'checkbox', checked: true, caption: 'Show Burnt Sienna upper horizontal cladding' },
...
const main = (params) => {
  ...
  // Front elevation only: three equal horizontal Oreo rows, top-to-bottom.
  // Manor Grey outside rows; Burnt Sienna wood-grain in the middle row.
  const frontRowH = eaveZ / 3
  parts.push(box([W, finishT, frontRowH], [W / 2, -finishT / 2, frontRowH / 2], PALETTE.manorGrey))
  parts.push(box([W, finishT, frontRowH], [W / 2, -finishT / 2, frontRowH * 1.5], PALETTE.burntSienna))
  parts.push(box([W, finishT, eaveZ - 2 * frontRowH], [W / 2, -finishT / 2, (2 * frontRowH + eaveZ) / 2], PALETTE.manorGrey))
  // Garage-door elevation: retain the two-tone upper/lower split.
  parts.push(box([W, finishT, upperFloorZ], [W / 2, D + finishT / 2, upperFloorZ / 2], PALETTE.manorGrey))
  parts.push(box([W, finishT, eaveZ - upperFloorZ], [W / 2, D + finishT / 2, (upperFloorZ + eaveZ) / 2], PALETTE.burntSienna))
  // Driveway/man-door side retains the matching split; opposite side remains all Manor Grey.
  ...
  // Gable fields follow the outside row's Manor Grey material.
  parts.push(prismAlongY([[0, eaveZ], [W, eaveZ], [W / 2, ridgeZ]], -finishT, 0, PALETTE.manorGrey))
  parts.push(prismAlongY([[0, eaveZ], [W, eaveZ], [W / 2, ridgeZ]], D, D + finishT, PALETTE.burntSienna))
  ...
  // Horizontal Burnt Sienna board joints and entry portal accents.
  if (params.showAccent) {
    ...
    parts.push(...horizontalBoardsY(W / 2, -finishT - 0.016, upperFloorZ, W, eaveZ - upperFloorZ))
    parts.push(...horizontalBoardsY(W / 2, D + finishT + 0.016, upperFloorZ, W, eaveZ - upperFloorZ))
  ...
  if (params.showProfile) {
    ...
    parts.push(...profileRibsFrontBack(W, D + 0.088, 0, upperFloorZ))
    parts.push(...profileRibsFrontBack(W, D + 0.088, upperFloorZ, eaveZ))
    parts.push(...profileRibsSides(D, -0.088, 0, upperFloorZ))
    parts.push(...profileRibsSides(D, -0.088, upperFloorZ, eaveZ))
