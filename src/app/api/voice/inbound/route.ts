...
export async function GET() {
  return NextResponse.json({ ok: true, enabled: Boolean(process.env.TWILIO_AUTH_TOKEN?.trim()), forwardTo: process.env.VOICE_FORWARD_TO ? "set" : "not set", configure: "Twilio → your number → Voice → 'A call comes in' → POST this URL" });
}
