...
export function normalizeStore(raw: Partial<AppData>): AppData {
  ...
  return {
    ...
    knockTodos: (raw.knockTodos ?? seed.knockTodos).map((t) => ({
      ...t,
      calendarEventId: t.calendarEventId ?? null,
      reminderSentAt: t.reminderSentAt ?? null,
    })),
    knockProposals: (raw.knockProposals ?? seed.knockProposals).map((p) => ({
      ...p,
      taxRate: p.taxRate ?? 0,
      notes: p.notes ?? "",
      status: p.status ?? (p.signedAt ? "signed" : "draft"),
    ...
    knockCalendarEvents: raw.knockCalendarEvents ?? seed.knockCalendarEvents,
    webhookEndpoints: raw.webhookEndpoints ?? seed.webhookEndpoints,
    webhookDeliveries: raw.webhookDeliveries ?? seed.webhookDeliveries,
    pushSubscriptions: raw.pushSubscriptions ?? seed.pushSubscriptions,
