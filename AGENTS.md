...
- **Books & P&L:** `/admin/books` — local 2-year P&L from CRM data + **first-class QuickBooks Online** (`QUICKBOOKS_*` env or paste credentials; Node host required for live QB API). Mainframe `qb_*` tools sync customers, invoices, payroll TimeActivity.
- **Mainframe AI:** `/admin/assistant` or floating MAINFRAME button — full CRM tools (leads, jobs, invoices, tasks complete/list, automations, hunt, QuickBooks). Outreach always `pending_approval`. Optional `GEMINI_API_KEY` (preferred) or `OPENAI_API_KEY` for NLU; local command parser fallback. Side panel lists automations + audit; `GET /api/ai/status` reports active provider.
- **Admin ↔ Field:** Field modes CTA switches to `/apps`; **Ops** return is always visible on mobile bottom bar + topbar when the user has dashboard access. Jarvis deep links stay on `/admin/*` twins while browsing admin.
