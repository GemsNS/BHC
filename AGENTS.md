...
- Static demo localStorage key: `bhc-crm-store-v7` (bump when `AppData` schema changes). **Do not** pass `withBasePath()` to Next.js `Link` or `router.push` — Next adds `basePath` automatically; use `withBasePath` for `fetch` only.
...
- **Active Knocker:** `/apps/knocker` (field) · `/admin/knocker` (command) — map turfs, color pins, GPS breadcrumbs, double-knock sync, route handoff, tasks, chat, leaderboard. API: `GET/POST /api/knocker`.
